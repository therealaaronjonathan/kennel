        ...
        <div className="max-w-2xl mx-auto px-6 py-8 space-y-6">
          ...
          {loading ? (
            ...
            <div className="rounded-[4px] border border-border-base overflow-hidden divide-y divide-border-base">
              {doctors.map((doctor) => (
                <div key={doctor.id} className="flex items-center justify-between px-5 py-4 bg-surface">
                  <div className="space-y-0.5">
                    <p className="text-[13px] font-semibold text-foreground">{doctor.name}</p>
                    {doctor.email && (
                      <p className="text-[12px] text-muted">{doctor.email}</p>
                    )}
                    {doctor.specialization && (
                      <p className="text-[11px] text-muted">{doctor.specialization}</p>
                    )}
                  </div>
                  <span
